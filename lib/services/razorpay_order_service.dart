...
Future<String?> createRazorpayOrder(int amountPaise) async {
  ...
  try {
    final uri = Uri.parse('$_baseUrl/webservice.asmx/CreateRazorpayOrder').replace(
    ...
    try {
      ...
      if (decoded is Map<String, dynamic>) {
        ...
        if (s != null && s.isNotEmpty && s.toLowerCase().startsWith('order_')) return s;
      }
    } catch (_) {}
    // Plain text order_id
    final trimmed = cleaned.trim();
    if (trimmed.toLowerCase().startsWith('order_')) return trimmed;
    return null;
  } catch (_) {
    return null;
  }
